...
CREATE TABLE network(gid serial, osm_id INTEGER, name VARCHAR, the_geom GEOMETRY, source BIGINT, target BIGINT, length FLOAT);
...
SELECT pgr_createtopology('network', 0.00002, 'the_geom', 'gid', 'source', 'target');
-- SELECT pgr_createtopology('network', 0.00002, 'the_geom', 'gid', );
